...
- **Phase E Wave 0–1 position domain（2026-07-10）**：`Book` 成為持倉 mutation SSOT（`apply_entry_fill` / `apply_exit_leg` / `adopt_broker_position` / `to_position_snapshot`）；broker 讀寫抽至 `position_sync.py`；週期 drift 抽至 `reconcile.py`；`SessionMixin` 只留 login/CA/contract。Engine 移除 `build_*_audit` wrapper；reconnect alert 改 `subscribe/trade`（不再寫 ATR）。`trailing_peak` 文件化為 legacy bag（不做 trailing 邏輯）。Wave 2–3（connectivity WD / order 切塊）待續。
- **Host alpha strip（2026-07-10）**：Host 不再持有策略指標（ATR/VWAP/momentum/trend/structure）；`MarketSnapshot` 僅 `ts/price/dt`；`RiskGate` 移除 `atr_stale`/`structure_stale`；`Strategy.evaluate(market, position, risk)`；`indicators.py` 與 trend/structure refresh 移至 `legacy/`；reconnect 健康門不再依賴 `refresh_atr`。
- **Trust API kbars + rollover ownership（2026-07-10）**：移除 active `kbar_repair` / `tick_minute_bars`（不再用 ticks 補 kbar）；`tick_rollover` 移至 `backfilldata/`。Live 與 fetch 寫入模型維持分開（共用 `tick_cache` CSV）。
- **Slim storage + backfilldata（2026-07-10）**：tick 檔 `{code}_{D}.csv` = **曆日 D only**（`fetch_calendar_day_ticks`：AllDay D∪D+1 後 filter）；空日 skip 不寫檔。`backfilldata` CLI 僅 `date`/`month` + `--code`/`--tick-cache-dir`/`--overwrite`/`--uat`/`--production`（預設 UAT）；無 holiday calendar。`cache_audit` / `cache_repair` / `legacy_cache_migrate` / `taiwan_calendar` → `legacy/`。`SessionBarCache` disk-only `trading_days`（移除 calendar soft tip）。
- **Lean Host Phase A（2026-07-09）**：`config.yaml` 精簡為 simulation / product / simple strategy / session / logging / operations；`SimpleStrategy` 不再因 `atr_stale` 擋 UAT flip；`plugins.load_strategy` stub→`NotImplementedError`（改直接注入）；研究型 engine tests（volume/overlay/regime/structure_stale/session_flatten/state_snapshot）移至 `legacy/apps/trading-app/tests/engine/`。
- **CSV-only tick_cache（2026-07-09）**：移除 `.csv.gz` 讀寫／壓縮／解壓 CLI；`tick_cache` 僅 plain CSV。`post-session.sh` 不再 gzip tick／ops log。UAT flip：**刻意不** Host-latch `max_daily_loss_points`（僅 `max_consecutive_loss`）。
- **UAT flip strategy（2026-07-09）**：`SimpleStrategy` 改為成交後 `flip_interval_sec`（預設 300）翻倉（flat→Buy / long→Sell）；`session_force_flatten_signal` 回 `None` 讓 Host 用 `flatten_slippage_points`。不 latch 日損。
- **Bugbot follow-up（2026-07-09）**：`SimpleStrategy` 僅在確認持倉後標記 session（IOC miss 可重試）；`is_trading_session` 加 weekday gate（日盤 Mon–Fri；夜盤晚間 Mon–Fri / 凌晨 Tue–Sat）；`pyproject.toml` 安裝 flat modules（`config` / `strategy_simple` 等）使 `pip install -e` 無需手動 `PYTHONPATH`。
- **Merge + bugfix（2026-07-09）**：`packages/trading-engine` 併入 `apps/trading-app/src/trading_engine`（單一永豐產品）。Bugbot 修復：gap sticky force_flatten；TAIFEX 交易日 15:00 共用風控預算；kernel 執行 `max_consecutive_loss`；`strategy_simple` 跟 config session 邊界。Live 直接建構 `SimpleStrategy`（code-to-interface）。重寫 [`docs/AGENTS.md`](docs/AGENTS.md)。
- **Refresh / base reset（2026-07-09）**：研究產物（strategies、trading-backtest、reporting、sweep、backtest、scripts、gudt、features/UAT/TODO/WeeklyStatus）`git mv` 至 [`legacy/`](legacy/README.md)。Active 路徑為 `apps/trading-app`（後續再併入 Host）。預設策略 `simple`（每 session 一多單 → 固定 TP/SL → 強平）；`config.yaml` 啟用日盤+夜盤；kernel 支援 overnight session / flatten。`performance_metrics` 救出為 `apps/trading-app/src/metrics.py`。
- **FT-023 GUDT Wash Beta UAT（2026-07-02）**：`strategy.name: gudt_wash_beta` — wash 日 open→session flatten sleeve（與 Route A 並存）；workspace [`gudt-wash-beta-baseline`](workspaces/gudt-wash-beta-baseline/) · 執行 parity **44/44**（2026 H1）· live `_session_bars` 對齊 probe（夜盤 kbar 不再誤判 `no_open_0845`）；[`gudt-wash-beta/SPEC.md`](legacy/docs/features/gudt-wash-beta/SPEC.md)。
...
- **Exit L3 infer-clear double-exit / over-flatten (2026-06-29 live RCA)**: Exit pending was cleared when `list_positions` showed an unchanged position, allowing a second stop-loss while the first IOC could still fill → kernel Flat vs broker Long. Removed exit L3 infer-clear entirely (symmetric with entry). Added `exit_miss_confirm_sec` (default 5) + `_resolve_exit_missed` (stop_loss → market flatten; take_profit/trailing → clear + retry). **Severe drift** (`kernel=Flat` + `broker_qty>0`, direction reversal, ceiling breach) now HALTs + market converge (was block-only for Flat/Long), with `reconcile_confirm_reads` debounce on periodic reconcile. **Post-exit fast reconcile** (`post_exit_reconcile_sec`, default 15). **Cleared-order registry** (`cleared_order_registry_sec`, default 120) for late deal attribution — registry only via `_clear_pending(watch_late_fill=True)`, not after normal fills. **Stop-loss miss race**: `_validate_order_signal` rejects strategy orders while `_stop_market_flatten_request` is set. Tests: `test_exit_single_flight.py`, `test_truth_driven_execution.py`, `test_position_reconcile.py`.
...
- **FT-019 Phase 0**：`reporting/simulate_fvg_mid_trail_skew_exit.py` · `sweep_fvg_breakout_trail_counterfactual.py` · `scripts/ft019_sfbt_counterfactual.py`；測試 T1–T8 + CF case 1–12；`post_entry_diagnosis` 支援 `fvg_mid_trail_sim`。
