...
> Archived from separate-repo era. Current development is in the `tfx-trading` monorepo under `packages/strategies/vwap-momentum/`. Install via monorepo clone + editable or local paths (see root README + monorepo/SPEC.md). Old git+ URLs no longer active for new work.
...
- See current monorepo `monorepo/SPEC.md` and root `SPEC.md` for modern architecture (this note is historical)
