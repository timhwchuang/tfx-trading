...
class TestOrderSmoke(unittest.TestCase):
    ...
    def test_consecutive_loss_does_not_freeze_capital(self):
        ...
        host._arm_pending(entry)
        host.handle_order_event(FUTURES_ORDER, _order_new("cl-buy-1"))
        host.handle_order_event(FUTURES_DEAL, _deal("cl-buy-1", action="Buy", price="18000"))

        exit_sig = OrderSignal("Sell", 1, 17990.0, "exit", exchange_ts=200, signal_id="cl-x1")
        host._arm_pending(exit_sig)
