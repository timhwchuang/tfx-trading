...
**當前階段（2026-06-17）**：已遷移至 **`timhwchuang/tfx-trading`** monorepo；Phase 7 + P0/P4-13 已落地；**待永豐模擬 API 金鑰**後開 UAT。見 `TODO.md` + `docs/WeeklyStatus.md`（最新一節）；架構見 [`docs/Architecture.md`](../../docs/Architecture.md)。
...
- **`docs/` 整個資料夾**（索引見 [`docs/README.md`](docs/README.md)），尤其是：
  ...
  - `AuditContract.md`、`UAT_CHECKLIST.md`（含整合後的 Pilot Readiness Gate）、`WindowsOps.md`、`CALLBACK_GUARDRAILS.md`。
  - Kernel UAT 引用 [`packages/trading-engine/docs/UAT_CHECKLIST.md`](../../packages/trading-engine/docs/UAT_CHECKLIST.md)，勿在 app 重寫。
...
| 秒停損率等 KPI **觀測**，非 UAT 通過條件 | `UAT_CHECKLIST.md` (Pilot Gate) |

### 4.2 Pilot / Live Gate（人類決策，Agent 不可自行宣告通過）

| 條件 | 參考 |
| ---- | ---- |
| UAT 連續數日狀態機零異常 | `UAT_CHECKLIST.md` (Pilot Gate) §二 |
| **P2-7 秒停損率**達標（Pilot 硬指標） | `UAT_CHECKLIST.md` (Pilot Gate)、`uat_report.py` |
| 正式 CA + `simulation: false`（**僅人類操作**） | `UAT_CHECKLIST.md` (Pilot Gate)、`README.md` |
| `LOG_LEVEL=INFO`；禁止 on_tick DEBUG | `UAT_CHECKLIST.md` (Pilot Gate) §二 |
| Phase 6 旗標：須 UAT tick 回測校準 `k` 後才開 | `TODO.md` Phase 6、`param_sweep` |
| Telegram / Webhook **實機**收得到告警 | `WindowsOps.md` P4-3 |
| 斷網 / No-tick 看門狗 / 重連對帳手動驗過 | `UAT_CHECKLIST.md` (Pilot Gate)、`WindowsOps.md` |
| Pilot 固定 **qty=1**，2–4 週不調參 | `UAT_CHECKLIST.md` (Pilot Gate) §三 |
| 券商帳戶損益與 log 點數每日對帳 | `UAT_CHECKLIST.md` (Pilot Gate) §三 |
...
├── SPEC.md                           # 整合 SPEC 入口
├── docs/Architecture.md              # 架構真相（模組邊界、資料流）
├── docs/DOC_MAP.md                   # 全文件索引
├── scripts/
│   ├── setup-dev.sh                  # 建立 .venv + editable install（CI / 開發必跑）
│   └── run-all-tests.sh              # 四 package 測試
├── packages/
│   ├── trading-engine/               # import trading_engine — 狀態機、Protocol
│   ├── trading-backtest/             # import trading_backtest — MockBroker、replay
│   └── strategies/
│       └── vwap-momentum/            # strategy_vwap_momentum plugin
└── apps/
    └── trading-app/                  # 本 AGENTS.md 所在 — Windows 整合層
        ├── config/config.yaml
        ├── scripts/windows/          # start-trading-app.ps1（-MonorepoRoot C:\tfx-trading）
        ├── src/                      # live / backtest / integrations / storage / reporting / sweep
        ├── tests/                    # 整合測試（81 項）
        ├── docs/                     # UAT、WeeklyStatus、WindowsOps
        └── run_tests.py
```
...
- 詳見 [`docs/Architecture.md`](../../docs/Architecture.md) 與各 package `SPEC.md`。
...
- UAT 觀測：高頻 tick 下 lock 等待是否曾 **>50ms**（見 `UAT_CHECKLIST.md` (Pilot Gate)）。
...
| 斷線 / No-tick | P4-8 看門狗 → 停新進場、允許平倉、重連後對帳順序見 `UAT_CHECKLIST.md` (Pilot Gate) |
...
5. 讀 `packages/trading-engine/docs/STRATEGY.md`（Protocol 契約）。
6. 讀 `packages/trading-engine/src/trading_engine/engine.py` 前 150 行 + `apps/trading-app/src/backtest/engine.py`。
7. 讀 [`docs/DOC_MAP.md`](../../docs/DOC_MAP.md) + `docs/BackTestingSpec.md`（索引 stub）。
8. 看 `tests/test_helpers.py` 的 `make_host`。
9. 確認 `run_tests.py` 內 maintenance note。
10. 若任務涉及上線：讀 `UAT_CHECKLIST.md` (Pilot Gate) + `WindowsOps.md`。
