"""FT-019 Phase 0: Sweep FVG breakout trail counterfactual CLI."""
...
from reporting.sweep_fvg_breakout_trail_counterfactual import (
    ...
    FINGERPRINT_FVG_AGE,
    FINGERPRINT_HARD_TP,
    FINGERPRINT_RECLAIM_SEC,
    FINGERPRINT_SWEEP_K,
    FINGERPRINT_SWEEP_LB,
    FINGERPRINT_SWING_LB,
    FINGERPRINT_TRAIL_ARM_ATR,
    FINGERPRINT_TRAIL_ARM_RISK,
...
def _write_gate_report(
    ...
    *,
    ...
    if mode == "fingerprint":
        ...
        lines.extend(
            [
                ...
                "",
                f"- session_days={funnel.get('days_with_session')} → sweep_signal={funnel.get('sweep_signal')} → "
                f"reclaim_ok={funnel.get('reclaim_ok')} → fvg_active={funnel.get('fvg_active')} → "
                f"breakout_signal={funnel.get('breakout_signal')} → entry={funnel.get('entry')}",
                "",
            ]
        )
        ...
        lines.extend(
            [
                ...
                f"- **pct_hit_2R** = {exit_fp.get('pct_hit_2R', '—')}",
        ...
        fp_path = path.parent / "reports" / "counterfactual_sfbt_fingerprint.json"
        fp_summary = ""
        if fp_path.is_file():
        ...
        lines.extend(
            [
                "## Fingerprint (0c-1)",
                "",
                f"（見 `counterfactual_sfbt_fingerprint.json`）· {fp_summary or '—'}",
                "",
                "## Grid (0c-2)",
                "",
            ]
        ...
        lines.extend(
            [
                "### Exit 診斷（best param）",
                "",
                f"- exit_gap={exit_best.get('exit_gap')} · pct_hit_2R={exit_best.get('pct_hit_2R')}",
                "",
            ]
        )
    ...
    if mode == "fingerprint":
        ...
        best_key = (t_gate.get("best_passing") or {}).get("param")
        skew_best = (train_payload.get("skew_gate_by_param") or {}).get(best_key or "", {})
        if skew_best.get("disqualified"):
            lines.append("| 決策 | **MVPClosed** — `sfbt_no_skew_champion` |")
            outcome = "sfbt_no_skew_champion"
        else:
            lines.append("| 決策 | **Go Phase 1 候選** — grid 通過 · 待人類 Go |")
    else:
        lines.append("| 決策 | **MVPClosed** — `sfbt_fingerprint_pass_g1_fail` |")
...
def main(argv: list[str] | None = None) -> int:
    root = _repo_root()
    ws = root / "workspaces" / "sfbt-baseline"
    reports = ws / "reports"
    parser = argparse.ArgumentParser(description="FT-019 SFBT counterfactual")
    ...
    train_payload = build_sfbt_payload(from_date=args.train_from, to_date=args.train_to, **common)
    valid_payload = build_sfbt_payload(from_date=args.valid_from, to_date=args.valid_to, **common)

    reports.mkdir(parents=True, exist_ok=True)
    train_path = (
        reports / "counterfactual_sfbt_train.json"
        if mode == "grid"
        else reports / "counterfactual_sfbt_fingerprint.json"
    )
    valid_path = reports / "counterfactual_sfbt_valid.json"
