"""FT-023: parity check — wash-beta CF vs bootstrap day plans."""
...
from reporting.wash_beta_parity_report import build_wash_beta_parity_report, write_json_report
...
def _failure_day(msg: str) -> str | None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="FT-023 wash-beta parity check")
    ...
    report = build_wash_beta_parity_report(
        cfg,
        code=args.code,
    ...
    if date_range.months:
        ...
        mismatch_failures = {
            f"{m.get('day')}: {m.get('reason')}" for m in report["mismatches"]
        }
        aggregate_failures = [
            f for f in all_failures if _failure_day(f) is None
        ]
        report["failures"] = aggregate_failures + sorted(mismatch_failures)
