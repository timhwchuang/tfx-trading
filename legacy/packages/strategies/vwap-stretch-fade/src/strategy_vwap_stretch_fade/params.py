"""Strategy parameters for vwap-stretch-fade plugin."""
...
class StretchFadeParams:
    ...
    @property
    def stretch_k(self) -> float:
        return float(self._live("STRETCH_K", "stretch_k", 2.0))

    @property
    def reset_z(self) -> float:
        return float(self._live("RESET_Z", "reset_z", 0.5))

    @property
    def cooldown_sec(self) -> int:
        return int(self._live("COOLDOWN_SEC", "cooldown_sec", 60))
    ...
    @classmethod
    def from_runtime_config(cls, cfg: RuntimeConfig) -> StretchFadeParams:
        return cls(_cfg=cfg)
