"""Taiwan holiday calendar (pin-yi) → trading-day filter.

Lives under ``storage`` so bar-cache and backfill share one implementation without
``storage → backfilldata`` imports. Only depends on ``storage.cache_paths`` (paths)
and the stdlib — not tick/kbar loaders — to stay import-cycle safe.
"""
...
class CalendarError(RuntimeError):
    """Taiwan trade-day calendar failure (API, cache, range, or parse)."""


def _date_range(start: datetime.date, end: datetime.date) -> list[datetime.date]:
    """Inclusive calendar-day range (local helper — avoids tick_loader import)."""
    days = (end - start).days
...
def parse_month_arg(month_token: str) -> tuple[int, int]:
    ...
    if len(raw) != 7 or raw[4] != "-":
        raise CalendarError(f"無效月份: {month_token!r}（需 YYYY-MM）")
    try:
        year = int(raw[:4])
        month = int(raw[5:7])
    except ValueError as exc:
        raise CalendarError(f"無效月份: {month_token!r}（需 YYYY-MM）") from exc
    if not 1 <= month <= 12:
        raise CalendarError(f"無效月份: {month_token!r}（月份需 01–12）")
    return year, month
...
def _parse_calendar_payload(data: Any, *, source: str) -> list[dict[str, Any]]:
    if not isinstance(data, list):
        raise CalendarError(f"Taiwan 行事曆 {source} 格式異常（預期 JSON array）")
    if not data:
        raise CalendarError(f"Taiwan 行事曆 {source} 無資料（year 可能不支援或 API 異常）")
    return data
...
def load_taiwan_calendar_year(
    ...
    *,
    ...
    try:
        ...
        raise CalendarError(f"Taiwan 行事曆快取 {path.name} 非 JSON") from exc
...
def fetch_taiwan_calendar_year(
    ...
    *,
    ...
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "tfx-trading-storage/1.0",
    ...
    try:
        ...
        raise CalendarError(f"Taiwan 行事曆 API 無法連線: {exc}") from exc
    try:
        data = json.loads(payload.decode("utf-8"))
    except json.JSONDecodeError as exc:
        raise CalendarError("Taiwan 行事曆 API 回傳非 JSON") from exc
...
def resolve_trading_days_in_range(
    ...
    *,
    ...
    if end < start:
        raise CalendarError(f"結束日 {end} 早於開始日 {start}")

    range_days = _date_range(start, end)
