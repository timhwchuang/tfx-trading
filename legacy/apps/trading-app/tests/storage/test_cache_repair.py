...
from backfilldata.tick_rollover import (
...
class TestKbarRepair(unittest.TestCase):
    ...
    def test_repair_writes_plain_csv(self):
        day = datetime.date(2026, 1, 8)
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            ticks = [
            ...
            self.assertFalse(Path(str(plain) + ".gz").is_file())
    ...
    def test_merge_afternoon_into_primary(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with unittest.mock.patch(
                "backfilldata.tick_rollover.fetch_ticks_for_date", return_value=afternoon
            ):
