...
- **Live 相關除錯**：由人類在 **GCE 或 Windows Live 節點**執行；Agent 只改 code + 測試 + 文件，並給出 shell/PowerShell 指令讓人類複製執行。
...
- **`docs/`**（索引見 [`DOC_MAP.md`](DOC_MAP.md)），尤其是：
  ...
  - Audit / sweep：同上 app SPEC §Integration contracts；[`uat/APP.md`](uat/APP.md)（含 Pilot Gate）、[`ops/HYBRID_DEPLOY.md`](ops/HYBRID_DEPLOY.md)、[`ops/LinuxOps.md`](ops/LinuxOps.md)、[`ops/WindowsOps.md`](ops/WindowsOps.md)。
...
| 條件 | 參考 |
| ---- | ---- |
| `bash scripts/run-all-tests.sh` 全綠（**269**：app **121** + packages；見 monorepo 根）或 app 內 `python run_tests.py` | 本檔 §10 |
...
| Telegram / Webhook **實機**收得到告警 | [`ops/LinuxOps.md`](ops/LinuxOps.md) / [`ops/WindowsOps.md`](ops/WindowsOps.md) P4-3 |
| 斷網 / No-tick 看門狗 / 重連對帳手動驗過 | [`uat/APP.md`](uat/APP.md)、[`ops/HYBRID_DEPLOY.md`](ops/HYBRID_DEPLOY.md) |
...
4. 跑測試確認基線（見 §10；monorepo `bash scripts/run-all-tests.sh` 或 app **121** 項）。
...
CI 已落地：monorepo 根 [`.github/workflows/ci.yml`](../../.github/workflows/ci.yml)（`bash scripts/run-all-tests.sh`）。**本地 merge 前仍須全綠**（四 package；合計 **269**）。
...
| 進程守護 | GCE：`scripts/linux/install-systemd.sh`；Windows：`register-task.ps1` / NSSM（[`ops/HYBRID_DEPLOY.md`](ops/HYBRID_DEPLOY.md)） |

### 9.3 日誌、tick 落盤與容量

- Log：`LOG_FILE` + 每日輪替（P4-2）；結構化審計行見 [`apps/trading-app/SPEC.md`](../apps/trading-app/SPEC.md) §Integration contracts。
- Tick：`tick_cache/{code}_{date}.csv` → 收盤後 `python -m storage` → `*.csv.gz`（**預設排除當日**；`storage.compress` 為相容 alias）。缺日可收盤後 `python -m backfilldata date YYYY-MM-DD`（勿與 live 同時 login）。跨月結算日 R1 尾盤由 backfill 自動合併 R2；回測前可 `python -m storage.cache_audit` / `cache_repair --fix` 驗證 tick×kbar。
...
# Live（模擬）— 僅人類在 GCE / Windows Live 節點執行，見 §2
cd apps/trading-app/src && python -m live
# Linux/GCE：bash scripts/linux/start-trading-app.sh
...
10. 若任務涉及上線：讀 [`uat/APP.md`](uat/APP.md) Phase 5–7 + [`ops/HYBRID_DEPLOY.md`](ops/HYBRID_DEPLOY.md)。
...
- **地雲雙管**：Live 建議 **GCE asia-east1** 或 **Windows**（[`ops/HYBRID_DEPLOY.md`](ops/HYBRID_DEPLOY.md)）；回測/CAL 放地端 Linux/macOS。運維 SSOT：[`ops/LinuxOps.md`](ops/LinuxOps.md)、[`ops/WindowsOps.md`](ops/WindowsOps.md)。
