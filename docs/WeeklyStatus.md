...
**決策**
- **UAT 持續** = 狀態機 / fill / 對帳；**不驗**策略獲利。
- **主 focus** = Alpha 新 thesis（FT-012+）；現有策略回測均已知不佳。
- **P6-1 / P6-SMC CAL-8** → **放棄**（濾網綁定 `grid_no_viable_solution` 的 vwap-momentum）。
- **Pilot 阻塞**，直至新 alpha 過 v2.1 train gate。

**文件**
- [`strategy_diagnosis.md`](../workspaces/strategy_diagnosis.md) §8 · [`TODO.md`](TODO.md) §已放棄 · [`uat/APP.md`](uat/APP.md) 橫幅

**備註**
- 下方歷史條目若提及「Go Pilot-prep」或 CAL-8 待辦，僅作**當日快照**；以本節為準。
...
## P6-SMC-CAL 報表模板（**封存** — CAL-8 已放棄）

> **⛔ 2026-06-28**：不再填寫、不跑 harness 簽核。以下模板僅作歷史參考。
...
- ~~[ ] ≥5 交易日後跑 `structure_calibration_cli` + 填本檔 **P6-SMC-CAL 模板** → CAL-8 簽核~~ — **已放棄**
- ~~[ ] 若 CAL-8 前要做 filter-on 演練~~ — **已放棄**

**Pending / 待決策**
- ~~CAL-8 Go/No-Go~~ — **已放棄**（2026-06-28）
- ~~Phase 5 Land：§9 跨 package SPEC 併入~~ — FT-002 MVPClosed
...
| **KBARS_ARCHIVE** | 建議 UAT 一併開啟，供 ATR 熱身 / HTF 回測（`tick_cache/*_kbars_*`）。 |
| **P6-SMC-CAL** | **⛔ 已放棄**（2026-06-28）— 見 [`strategy_diagnosis.md`](../workspaces/strategy_diagnosis.md) §8.2 |
| **UAT 執行** | Phase 0 開跑 → [`uat/APP.md`](uat/APP.md)；證據 [`uat_evidence/`](../uat_evidence/) |
| **Phase 6 CAL B 類** | **⛔ 已放棄** — P6-1 / P6-SMC；harness CLI 封存 |
