...
## Phase 4 實作 Re-Review（2026-06-18）

**範圍**：`regime_allows_entry` 策略接線、`structure_veto` audit、armed enrichment、`record_structure_veto`、uat_report、filter-on determinism

### 初審 → 修正 → 複審

| 初審 issue | 共識 | 處置 |
|------------|------|------|
| MEDIUM `momentum_armed` 序列化洩漏 structure 零值預設 | **同意** | `format_decision_audit`：armed 跳過 generic fallback loop |
| MEDIUM veto audit 綁定 `obs is not None` | **同意** | 永遠 `logger.info` DECISION_AUDIT；`record_*` 仍經 obs |
| MEDIUM `structure_params_from_strategy` 缺 mock 欄位 → TypeError | **同意** | trend 測試補 `structure_*` 預設 |
| LOW `active_fvg_side` 未進 MarketSnapshot | **延後** | SPEC SHOULD；harness 重算 as-of 補齊 |
| LOW engine/package SPEC §9 未更新 | **延後** | Phase 5 Land 同 PR |
...
- `bash scripts/run-all-tests.sh`：**全綠**
- log 含 `structure_veto` + `structure_algo_version` + `momentum_dir`
- `momentum_armed`（filter on）含 structure 戰場快照
- `structure_stale` flat entry → `risk_blocked` / `block_reason=structure_stale`
- filter on 3-run determinism hash 一致
- `structure_filter_enabled=false` 行為與 Phase 3 等價

**Phase 4 Verdict：`PASS`** — 可進 Phase 5 UAT 累積 + CAL-8；勿開 filter 直至 B-class 證據 + 人類簽核。